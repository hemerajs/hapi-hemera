...
async function register(server, options) {
  const createHemera = (!options.hemera || (typeof options.hemera.use === 'undefined') && (typeof options.hemera.act === 'undefined'))

  const settings = Hoek.applyToDefaults(internals.defaults, options)

  const hemera = createHemera ? new Hemera(Nats.connect(settings.nats), settings.hemera) : options.hemera;
