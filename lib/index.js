...
exports.register = function (server, options, next) {
  ...
  // these also need the basepattern (wasn't sure how to quickly do actions)
  const replyAct = function (pattern) {
    const extendedPattern = Object.assign(pattern, internals.getBasePattern(this.request, settings.basePattern, pattern))
    this.request.hemera.act(extendedPattern, (err, result) => {
      this.response(err || result)
    })
  }
  const handlerAct = function (route, options) {
    return function (request, reply) {
      ...
      const extendedPattern = Object.assign(message, internals.getBasePattern(request, settings.basePattern, message))

      request.hemera.act(extendedPattern, (err, result) => {
        reply(err || result)
      })
    }
  }
  ...
  if (options.decoratePrefix) {
    ...
    server.decorate('reply', `${options.decoratePrefix}Act`, replyAct)
    server.handler(`${options.decoratePrefix}Act`, handlerAct)
  } else {
    server.decorate('server', 'action', internals.action(server))
    server.decorate('reply', 'act', replyAct)
    server.handler('act', handlerAct)
  }
