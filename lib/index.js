...
const internals = {
  defaults: {
    ...
    basePattern: (request) => {},
    // prefix to avoid conflicts with senecajs during migration
    prefix: ''
  }
}
...
exports.register = function (server, options, next) {
  ...
  server.expose('prefix', settings.prefix)
  server.decorate('server', 'hemera', hemera)
  server.decorate('server', internals.toCamelCase(settings.prefix, 'action'), internals.action(server))
  ...
  server.decorate('reply', internals.toCamelCase(settings.prefix, 'act'), internals.replyAct)
  server.handler(internals.toCamelCase(settings.prefix, 'act'), internals.handlerAct)

  // gracefully shuutdown hemera when hapi is stopped
...
internals.toCamelCase = function (a, b) {
  if (a) {
...
internals.handlerAct = function (route, options) {
  return function (request, reply) {
    ...
    const actName = internals.toCamelCase(request.server.plugins['hapi-hemera'].prefix, 'act')

    return reply[actName](message)
  }
}
